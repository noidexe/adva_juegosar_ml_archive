...
def archiveYahooMessage(file, archiveFile, messageYear, format):
     try:
          ...
          f.close()
          print 'Yahoo Message: ' + str(file) + ' archived to: archive-' + str(messageYear) + '.html'
     except Exception as e:
          print 'Yahoo Message: ' + str(file) + ' had an error:'
          print e
...
def getYahooMessageYear(file):
    ...
    f1.close()
    jsonDoc = json.loads(fileContents)
    if jsonDoc.has_key('ygData'):
         emailMessageTimeStamp = jsonDoc['ygData']['postDate']
    else:
         emailMessageTimeStamp = 0.0
